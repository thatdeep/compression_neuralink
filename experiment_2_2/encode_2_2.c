...
void write_compressed_file(const char *filename, uint16_t *data, int num_samples) {
    ...
    for (int i = 0; i < samples.size; ++i) {
        int int_sample = (int)(samples.data[i]);
        ...
        samples.data[i] = sample_table[insertion_index].stack_index;
    }
    ...
    pack_and_write16(file, samples.data, samples.size, (int)powerOfTwoExponent(nextPowerOf2(sample_stacksize)));
