...
void read_compressed_file(const char *filename, int16_t **data, size_t *num_samples) {
    ...
    size_t d64_data_size, dr_data_size, sample_stacksize;
    ...
    UVector16 samples2 = {.data = (uint16_t *)malloc(sizeof(uint16_t) * samples2_size), .size = samples2_size};
    ...
    unpack_bits16(file, samples2.data, samples2.size, powerOfTwoExponent(nextPowerOf2(sample_stacksize)));
    ...
    for (int i = 0; i < samples2.size; ++i) {
        samples2.data[i] = sample_stack[samples2.data[i]];
    }

    *num_samples = dr_data_size + 1;
    UVector16 samples = {.data = decompress(&samples2, &d64_data2, &dr_data2, 6), .size = dr_data2.size + 1};
