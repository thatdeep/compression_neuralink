...
int16_t *read_compressed_file(const char *filename, size_t *num_samples) {
    ...
    *num_samples = diffs_size + 1;
    ...
    uint32_t *udata_32_bwt = (uint32_t *)malloc(sizeof(uint32_t) * (*num_samples));
    for (int i = 0; i < *num_samples; ++i) {
        udata_32_bwt[i] = (uint32_t)(udata[i]) & 0xFFFF;
    }
    uint32_t *udata_32 = bwt_inverse(udata_32_bwt, (int)(*num_samples), 65536, sentinel_index);
    for (int i = 0; i < *num_samples; ++i) {
        udata[i] = (uint16_t)(udata_32[i]);
    }
    free(udata_32);
    free(udata_32_bwt);
