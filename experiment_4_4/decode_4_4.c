...
int16_t *read_compressed_file(const char *filename, size_t *num_samples) {
    ...
    uint16_t overhead_value;
    uint8_t overhead_count = 0;
    fread(smaller_diff_occ_frame, sizeof(uint16_t), diff_codes_alphabet_size, file);
    for (int i = 0; i < diff_codes_alphabet_size; ++i) {
        diff_occ_frame[i] = smaller_diff_occ_frame[i];
        if (smaller_diff_occ_frame[i] == 65535) {
            overhead_count++;
            fread(&overhead_value, sizeof(uint16_t), 1, file);
            // printf("largest small diff occ value=%d, overhead=%d\n", diff_occ_frame[i], (uint16_t)(diff_occ_frame[i] - 65535));
            diff_occ_frame[i] += overhead_value;
        }
    }
    ...
    *num_samples = (size_t)temp_num_samples;
    uint16_t *samples_codes_recovered = (uint16_t *)malloc(sizeof(uint16_t) * (*num_samples));
    fread(samples_codes_recovered, sizeof(uint16_t), 1, file);
    ...
    fread(diffs_reading_stream.ms.stream, sizeof(uint8_t), read_total_bytesize, file);
    ...
    if (garbage_bitsize) {
        read_bits_memstream(&(diffs_reading_stream.ms), garbage_bitsize);
    }
    uint16_t *diff_codes_recovered = decode16(*num_samples - 1, read_total_bitsize, final_state, &(diffs_reading_stream.ms), diff_occ_frame, diff_codes_alphabet_size, quant_pow);
    for (int i = 0; i < (*num_samples - 1) / 2; ++i) {
        uint16_t temp = diff_codes_recovered[i];
        diff_codes_recovered[i] = diff_codes_recovered[*num_samples - i - 2];
        diff_codes_recovered[*num_samples - i - 2] = temp;
    }
    ...
    for (int i = 1; i < *num_samples; ++i) {
        // printf("i=%d, diff_code_recovered=%d\n", i, diff_codes_recovered[i - 1] - zero_index_ref);
        samples_codes_recovered[i] = samples_codes_recovered[i - 1] + diff_codes_recovered[i - 1] - zero_index_ref;
    }
    // printf("tek\n");
