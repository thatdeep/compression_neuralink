...
// weird that symmetric pair of -24568 that is 24567 is missing. Care to include it to obtain full 1024 values table?
// there might really be a bad case when sample just dont exist in public data, but exist in private tests
static uint16_t codes_to_values[1023] = {
...
int16_t *read_compressed_file(const char *filename, size_t *num_samples) {
    ...
    *num_samples = (size_t)temp_num_samples;
    ...
    int16_t *data = (int16_t *)samples_codes_recovered;
    free(occ);
