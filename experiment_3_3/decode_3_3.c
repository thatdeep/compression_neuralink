...
int16_t *read_compressed_file(const char *filename, size_t *num_samples) {
    ...
    int quant_pow;
    int sentinel_index;
    fread(&sentinel_index, sizeof(int), 1, file);
    ...
    uint32_t *udata_32_bwt = (uint32_t *)malloc(sizeof(uint32_t) * diffs_size);
    for (int i = 0; i < diffs_size; ++i) {
        udata_32_bwt[i] = (uint32_t)(diffs_recovered[i]);
    }
    uint32_t *udata_32 = bwt_inverse(udata_32_bwt, (int)(diffs_size), alphabet_size, sentinel_index);
    for (int i = 0; i < diffs_size; ++i) {
        diffs_recovered[i] = (uint8_t)(udata_32[i]);
    }
    free(udata_32);
    free(udata_32_bwt);
